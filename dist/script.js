...
function displayQuestionAndAnswer(question) {
    ...
    if (questionH1)
        questionH1.innerText = question.question;
    if (span)
        span.innerText = String(quizQuestionNumber + 1);
    answerElements.forEach((el, index) => {
        if (el)
            el.innerText = question.options[index];
    });
}
function checkAnswer(answer) {
    ...
    if (quizQuestionNumber < questionsArray.length) {
        ...
        if (scoreElement)
            scoreElement.innerHTML = correctAnswers.toString();
        if (endOfTheQuiz)
            endOfTheQuiz.style.display = "block";
    }
}
function startTheQuiz() {
    displayQuestionAndAnswer(questionsArray[0]);
    document.querySelectorAll(".answer1, .answer2, .answer3, .answer4").forEach(option => {
        option.addEventListener("click", (event) => checkAnswer(event.currentTarget));
    });
}
